...
def add_afk(server_id,user_id,message):
    con = sqlite3.connect("date.db")
    sql = f"insert into afk(server_id,user_id,message) values({server_id},{user_id},'{message}');"
    cur = con.cursor()
    cur.execute(sql)
    con.commit()
    con.close()

def remove_afk(server_id,user_id):
    con = sqlite3.connect("date.db")
    sql = f"delete from afk where server_id = {server_id} and user_id = {user_id}"
    cur = con.cursor()
    cur.execute(sql)
    con.commit()
    con.close()
...
def add_welcomechannel(guild_id,channel_id):
    con = sqlite3.connect("date.db")
    sql = f"insert into welcoming(guild_id,channel_id) values({guild_id},{channel_id})"
    cur = con.cursor()
    cur.execute(sql)
    con.commit()
    con.close()

def remove_welcomechannel(channel_id):
    con = sqlite3.connect("date.db")
    sql = f"delete from welcoming where channel_id = {channel_id}"
    cur = con.cursor()
    cur.execute(sql)
    con.commit()
    con.close()