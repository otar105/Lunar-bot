...
class Role(commands.Cog):
    ...
    async def on_ready(self):
        print('Role Command Loaded Succesfully')
    ...
    async def role(self,ctx, user : discord.Member, role : discord.Role):
        try:
            ...
            embed = discord.Embed(description=f"<a:tick:859734309073715250> Updated roles for {user.mention}, **+ {role}**", color=0x00FF00)
    ...
    @commands.command()
    @commands.has_permissions(manage_roles=True)
    async def unrole(self,ctx, user : discord.Member, role : discord.Role):
        try:
            await user.remove_roles(role)
            embed = discord.Embed(description=f"<a:deny:860216874163240961> Removed role from {user.mention}, **- {role}**", color=0x00FF00)
            embed.set_author(name=f"{ctx.author}",icon_url=f"{ctx.author.avatar_url}")
            embed.set_footer(text=f"Lunar • today {datetime.datetime.now().hour}:{datetime.datetime.now().minute}",icon_url="https://images-ext-2.discordapp.net/external/F08dR3mn0Eg3YLm2mTbyd4I52Z2XSffki_g-XlUqljg/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/859504107768250379/01ac1392401eecdee1380ba4d82062d0.webp?width=406&height=406")
            await ctx.channel.send(embed=embed)
        except:
